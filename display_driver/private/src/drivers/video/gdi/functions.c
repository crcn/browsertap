...
RESOLUTION getResolution() {
  RESOLUTION res;
  //REG_HANDLE handle = openReg(TEXT("Software\\Eyebrowse"));
  res.x = 3000;//getIntValue(&handle, TEXT("ResolutionX"), 1000);
  res.y = 2500;//getIntValue(&handle, TEXT("ResolutionY"), 700);
  //closeReg(&handle);
  return res;
}
...
DHPDEV GdiExample_DrvEnablePDEV(DEVMODEW *pdm, PWSTR pwszLogAddr, ULONG cPat, HSURF *phsurfPatterns, ULONG cjCaps, GDIINFO *pGdiInfo, ULONG cjDevInfo, DEVINFO *pDevInfo, HDEV hdev, PWSTR pwszDeviceName, HANDLE hDriver) 
{
    ...
    if(pDeviceData)
    {
...

        {
            ...
            pGdiInfo->ulHorzRes        = res.x;
            pGdiInfo->ulVertRes        = res.y;
        ...
        pDeviceData->pVideoMemory = EngMapFile(L"\\??\\c:\\video.dat", res.x*res.y*4, &pDeviceData->pMappedFile);
...
ULONG GdiExample_DrvGetModes(HANDLE hDriver, ULONG cjSize, DEVMODEW *pdm)
{
   ...
   else
   {
      ...
      res = getResolution();
       ...
       pdm->dmBitsPerPel       = 32;
       pdm->dmPelsWidth        = res.x;
       pdm->dmPelsHeight       = res.y;
...
HSURF GdiExample_DrvEnableSurface(DHPDEV  dhpdev)
{
    ...
    EngModifySurface(pDeviceData->hsurf, pDeviceData->hdev, HOOK_FILLPATH | HOOK_STROKEPATH | HOOK_LINETO | HOOK_TEXTOUT | HOOK_BITBLT | HOOK_COPYBITS, 0, (DHSURF)pDeviceData->pDeviceSurface, pDeviceData->pVideoMemory, res.x*4, NULL);
