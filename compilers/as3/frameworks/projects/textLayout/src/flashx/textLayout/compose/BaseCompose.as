	...
	public class BaseCompose
	{
		...
		*/
		...
		public function composeTextFlow(textFlow:TextFlow, composeToPosition:int, controllerEndIndex:int):int
		{
			...
			initializeForComposer(_flowComposer, composeToPosition, controllerEndIndex);
			...
			if (_startComposePosition > 0)
			{
				...
				_curElement = previousElement;
				_curElementStart = previousElement.getAbsoluteStart();
				...
				var startLine:TextFlowLine = _flowComposer.getLineAt(startLineIndex);
				_spaceCarried = startLine.spaceAfter;
				commitLastLineState(startLine);
				// advance through the parcels to find the column where composition will start
				var startParcel:int = startLine.columnIndex;
				for (var parcelIndex:int = 0; _parcelList.currentParcel && _parcelList.columnIndex < startParcel; ++parcelIndex)
					_parcelList.next();
				
				if (_blockProgression == BlockProgression.TB)
					_parcelList.addTotalDepth(startLine.y + startLine.ascent - _parcelList.top);
				else
					_parcelList.addTotalDepth(_parcelList.right - startLine.x);
				...
				startLineIndex = _flowComposer.findLineIndexAtPosition(_startComposePosition);
				for (; lineIndex < startLineIndex; ++lineIndex)
				{
					...
					if (!line.alignment || !startController.measureWidth)
					...
					else
					{
						...
						alignData.rightSideGap = getRightSideGap(line, paraFormat, line.alignment != null);
						alignData.leftSideGap = getLeftSideGap(line, paraFormat);
		...
		protected function composeParagraphElementIntoLines():Boolean
		{
			...
			for (;;)
			{
				...
				calculateLineAlignmentAndBounds(curLine, alignData);
		...
		{
			var rightSideGap:Number =  paraFormat.direction == Direction.LTR ? paraFormat.paragraphEndIndent : paraFormat.paragraphStartIndent;
		...
		private function calculateLineAlignmentAndBounds(curLine:TextFlowLine, alignData:AlignData):void
		{
			...
			var lineWidth:Number = _parcelList.explicitLineBreaks ? calculateLineWidthExplicit(curLine) : textLine.textWidth;
			
			var rightSideGap:Number = getRightSideGap(curLine, _curParaFormat, alignData != null);
			var leftSideGap:Number = getLeftSideGap(curLine, _curParaFormat);
